...
*/
// rzq: #ifdef AUP1
// rzq: /* Copyright 1985 Advanced Programming Institute, Ltd. */
// rzq: /*1-3*/
// rzq: 
// rzq: #include <stdio.h>
// rzq: 
// rzq: void syserr(const char *msg)  /* print system call error message and terminate */
// rzq: {
// rzq:     extern int errno, sys_nerr;
// rzq:     extern char *sys_errlist[];
// rzq: 
// rzq:     fprintf(stderr, "ERROR: %s (%d", msg, errno);
// rzq:     if (errno > 0 && errno < sys_nerr)
// rzq:         fprintf(stderr, "; %s)\n", sys_errlist[errno]);
// rzq:     else
// rzq:         fprintf(stderr, ")\n");
// rzq:     exit(EXIT_FAILURE);
// rzq: }
// rzq: /**/
// rzq: #else
// rzq: 
// rzq: #define __EXTENSIONS__
...
#include <netdb.h>
// rzq: 
// rzq: #if STD_UNIX_VERSION < SUV_SUS3
// rzq: #ifdef SOLARIS
// rzq: #undef __EXTENSIONS__
// rzq: #endif
// rzq: #endif
...
const char *errsymbol(int errno_arg)
{
    return "XXXX"; // rzq: get_macrostr("errno", errno_arg, NULL);
}
/*[]*/
char *syserrmsgline(char *buf, size_t buf_max, int errno_arg, EC_ERRTYPE type)
{
    ...
    if (errno_arg == 0)
        ...
        snprintf(buf, buf_max, "%s (%d: \"%s\")",
          /* rzq: get_macrostr(cat, errno_arg, NULL) */
          "XXXX", errno_arg,
